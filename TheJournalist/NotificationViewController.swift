...
class NotificationViewController: NavigationViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    override func viewWillAppear(animated: Bool) {
        ...
        print("notificationTable size ... \(self.notificationTable.frame.size.width)")
        print("view size ... \(self.view.frame.size.width)")
...
//        let tableFrame = self.notificationTable.frame
//        tableFrame.width = self.view.frame
//        self.notificationTable.frame =
        print("did appear notificationTable size ... \(self.notificationTable.frame.size.width)")
        print("did appear view size ... \(self.view.frame.size.width)")
    }
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        if dammyContent[indexPath.row]["actionType"] == 1{
            ...
            let cell = notificationTable.dequeueReusableCellWithIdentifier("NotificationNoVoteCell", forIndexPath: indexPath) as! NotificationNoVoteCell
